...
const Auth = () => {
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      setFormSubmitMessage("Verifying email address...")
      ...
      const response = await fetch("/api/verify-email", {
        ...
        body: JSON.stringify({
          email: registerEmail,
          firstName: registerUsername || "User",
        }),
      })
      ...
      if (!response.ok) {
        ...
        switch (data.type) {
          ...
          case "invalid_recipient":
            setFormSubmitMessage(`Email verification failed: ${data.error}`)
            break
          default:
            setFormSubmitMessage("Email verification failed. Please try again.")
        }
      ...
      // Success case
      if (data.success) {
        setFormSubmitMessage("Email verified successfully! You can now complete registration.")
        setIsEmailVerified(true)
      } else {
        // This case handles when the API returns 200 but not success:true
        setFormSubmitMessage("Email verification completed but with warnings.")
        setIsEmailVerified(false)
      }
    } catch (error) {
      console.error("Error during email verification:", error)
      setFormSubmitMessage("Email verification process failed. Please try again.")
      setIsEmailVerified(false)
    } finally {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      setFormSubmitMessage(error.message || "An unexpected error occurred. Please try again.")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    if (!isEmailVerified) {
      setFormSubmitMessage("Please verify your email before registering.")
      setIsSubmitting(false)
      return
    }
  ...
  return (
    <div
      className="relative flex justify-center items-center min-h-screen bg-gradient-to-br from-[#FAF4ED] to-[#EFE7DD] p-4"
      style={{ userSelect: "none" }}
    >
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        <Image src="/city-image.jpg" alt="City Background" layout="fill" objectFit="cover" priority quality={100} />
      ...
      >
        ...
        <div className="flex bg-[#FAF4ED] border-b border-[#E2D9CE]">
          <motion.button
            className={`flex-1 py-4 text-center font-medium text-lg relative ${
              activeTab === "login" ? "text-[#36302A]" : "text-[#86807A]"
            }`}
          ...
          <motion.button
            className={`flex-1 py-4 text-center font-medium text-lg relative ${
              activeTab === "register" ? "text-[#36302A]" : "text-[#86807A]"
            }`}
        ...
        <AnimatePresence mode="wait">
          {activeTab === "login" && (
            ...
            >
              <motion.p className="text-[#36302A] text-sm mb-6" variants={itemVariant}>
                Welcome back! Please enter your credentials to access your account.
              </motion.p>

              <motion.div className="space-y-5" variants={itemVariant}>
              ...
              <motion.div className="flex justify-between items-center mt-6" variants={itemVariant}>
              ...
              <AnimatePresence>
                {formSubmitMessage && (
                  <motion.div
                    ...
                    className={`text-sm mt-6 px-4 py-3 rounded-lg ${
                      formSubmitMessage.includes("successful")
              ...
              <motion.p className="text-sm text-[#36302A] text-center mt-6" variants={itemVariant}>
          ...
          {activeTab === "register" && (
            ...
            >
              ...
              <motion.div className="space-y-4" variants={itemVariant}>
                ...
                <div className="flex gap-2 items-center">
                  <motion.button
                    ...
                    className={`px-4 py-2 rounded-lg flex items-center transition-colors duration-300 ${
                      isEmailVerified ? "bg-green-50 text-green-700 border border-green-200" : "bg-[#36302A] text-white"
                    }`}
                    whileHover={{ scale: 1.03 }}
                    whileTap={{ scale: 0.97 }}
                    disabled={isVerifyingEmail || !registerEmail}
                  >
                    {isVerifyingEmail ? (
                ...
                {!isEmailVerified && registerEmail && (
                  <motion.div
                    initial={{ opacity: 0, height: 0 }}
                    animate={{ opacity: 1, height: "auto" }}
                    exit={{ opacity: 0, height: 0 }}
                    className="text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3 mt-3 text-sm"
                  >
              ...
              <motion.div className="flex items-start gap-2 mt-5" variants={itemVariant}>
                ...
                <label htmlFor="terms" className="text-sm text-[#36302A] cursor-pointer">
                  I agree to the{" "}
                  <a href="/terms" className="text-[#36302A] font-semibold hover:underline">
                    Terms of Service
                  </a>{" "}
                  and{" "}
                  <a href="/privacy" className="text-[#36302A] font-semibold hover:underline">
                    Privacy Policy
                  </a>
                </label>
              </motion.div>
              ...
              <motion.div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mt-5" variants={itemVariant}>
