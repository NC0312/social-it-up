...
export function AdminAuthProvider({ children }) {
  ...
  const register = async (email, username, password, gender = "unspecified", isEmailVerified) => {
    try {
      ...
      if (!isEmailVerified) {
        return { success: false, error: "Email verification is required before registration" }
      }
      ...
      const adminRef = await addDoc(collection(db, "admins"), {
        ...
        isEmailVerified: true, // Store that email was verified
      })
  ...
  return (
    <AdminAuthContext.Provider value={value}>
      ...
      {/* Admin Removal Popup */}
      <AnimatePresence>
        {showRemovalPopup && <AdminRemovalPopup onClose={handleRemovalPopupClose} reason={removalReason} />}
      </AnimatePresence>
